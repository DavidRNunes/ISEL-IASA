...
class No():
    ...
    def profundidade(self):
        """
        Propriedade que permite obter o valor da profundidade do nó a partir da profundidade
        dos antecessores - profundidade essa que está associada ao número de antecessores que
        um dado nó tem
        """
        return self._antecessor.profundidade+1 if self._antecessor else 0

    @property
    def custo(self):
        """
        Propriedade que permite obter o valor do custo da operação do nó, correspondente ao
        custo de chegar até ao nó, obtido através do custo dos nós antecessores do nó actual
        """
        if self._operador:
            _custo_operador = self._operador.custo(self._antecessor.estado, self._estado)
            return _custo_operador + self._antecessor.custo
        else:
            return 0
    ...
    def __lt__(self, no):
        """
        Método de comparação do Python que verifica se o primeiro objeto fornecido é menor
        do que o segundo objeto fornecido

        Através deste método podemos fazer a comparação entre dois nós para saber se o nó
        actual tem um custo menor do que o nó com o qual pretendemos comparar

        @param no: nó com o qual se pretende comparar o custo do próprio nó
        @returns: valor booleano da comparação entre dois nós, retorna true se o nó
            actual tiver um custo menor do que o nó com o qual estamos a comparar
        """
